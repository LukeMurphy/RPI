...
def main(run=True):
	...
	config.crackColor = tuple(
		map(lambda x: int(int(x) * config.brightness), config.crackColorVals)
	)
	...
	config.randomColorSampleProb = float(
		workConfig.get("screenproject", "randomColorSampleProb")
	)
	config.probDrawLines = float(workConfig.get("screenproject", "probDrawLines"))
	config.probDrawPerpLines = float(
		workConfig.get("screenproject", "probDrawPerpLines")
	)
	...
	config.origin = tuple(
		map(lambda x: int(int(x) * config.brightness), config.originVals)
	)
	...
	config.colOverlay.minSaturation = float(
		workConfig.get("screenproject", "minSaturation")
	)
	config.colOverlay.maxSaturation = float(
		workConfig.get("screenproject", "maxSaturation")
	)
	config.colOverlay.minValue = float(workConfig.get("screenproject", "minValue"))
	config.colOverlay.maxBrightness = float(
	...
	try:
		...
		config.probabilityMultiplierRange = float(
			workConfig.get("screenproject", "probabilityMultiplierRange")
		)
