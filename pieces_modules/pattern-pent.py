...
def getColorChanger(config,workConfig):
...
def drawPattern(config):
	...
	drawConcentricRings(config)
	# drawTheReal()


def drawTheReal(config):
...
def drawConcentricRings(config):
	offset = (0, 0)
	angleOffset = 0
	level = 0
	numChildren = 0
...
def main(config, workConfig, run=True):
	...
	config.f1 = getColorChanger(config,workConfig)
	config.f2 = getColorChanger(config,workConfig)
	config.f3 = getColorChanger(config,workConfig)
	config.f4 = getColorChanger(config,workConfig)
	config.f5 = getColorChanger(config,workConfig)
	config.f6 = getColorChanger(config,workConfig)
...
def setUp(config,workConfig):
	drawPattern(config)

'''
def runWork(config,workConfig):
...
def callBack(config,workConfig):
	return True
