...
'''
this can now use a sequence of blocks -- 
but they don't necessarily need to be re-drawn each cycle
unless they are also doing color transitions - which does not
seem to be called - just setup to hold colors

each block should actually be told specifically which palette to use
so it would be easier to mix palettes on the fly rather than rely on
the disturbances to do it

So the mekePatternSequence function should actually do a better job
controlling when the pattern changes based on parameters of minimum and
maximum block repetition and also set the palette for each block

This may mean making another version of the pattern_blocks module

Also might make more sense to just build each image once and not call each
of the block functions each cycle  -- so the mekePatternSequence might 
alos create a list/array of blocks to draw to the canvas 
...
def buildPalette(palette):
    global workConfig
    # palette = config.palettes[index]

    print(str("Loading palette {}").format(palette))
    ...
    colOverlay.currentColor = colorutils.getRandomColorHSVSaturated(
        minHue,
        maxHue,
        minSaturation,
        maxSaturation,
    ...
    colOverlay.bgColor = colorutils.getRandomColorHSVSaturated(
        minHue,
        maxHue,
        minSaturation,
        maxSaturation,
    ...
    linecolOverlay.currentColor = [200, 10, 10]
    linecolOverlay.currentColor = colorutils.getRandomColorHSVSaturated(minHue, maxHue, minSaturation, maxSaturation, minValue, maxValue)
    ...
    linecolOverlay2.currentColor = [10, 100, 10]
    linecolOverlay2.currentColor = colorutils.getRandomColorHSVSaturated(minHue, maxHue, minSaturation, maxSaturation, minValue, maxValue)
    ...
    config.palettes.append(_paletteObj)
...
# ------------------ BUILD AND DRAW IMAGE BY REPEATING BLOCKS -----------
def repeatImage(config, canvasImage):
    ...
    for c in range(0, config.cols):
        for r in range(0, config.rows):
            if _counter in config.skipBlocks:
                ...
                _block = config.blockSequence[_counter]
                config.patternModel = _block.patternModel
                config.rotateAltBlock = _block.rotateAltBlock
                config.colOverlay = _block.colOverlay
                config.linecolOverlay = _block.linecolOverlay
                config.linecolOverlay2 = _block.linecolOverlay2

                config.blockImage = Image.new("RGBA", (config.blockWidth, config.blockHeight))
                config.blockDraw = ImageDraw.Draw(config.blockImage)
                func = eval("pattern_blocks." + _block.patternModel)
                func(config)
                ...
                # config.transitionImage.paste(_temp, (c * config.blockWidth-c, r * config.blockHeight-r), _temp)
...
def mekePatternSequence(config):

    config.patternSequence = []
    config.blockSequence = []
    ...
    print(f"numberOfPatterns {numberOfPatterns}")
    ...
    _paletteObj  = config.palettes[math.floor(random.random() * len (config.palettes))]
    
    paletteChangeProb = .5
    patternChangeProb = .5
    _paletteChangeProb = paletteChangeProb * totalSlots/100
    _patternChangeProb = patternChangeProb * totalSlots/100
    
    for i in range(0, totalSlots):

        # print(str("iterateCount count: {}").format(iterateCount))
        if random.random() < _patternChangeProb :
            pattern = config.patterns[math.floor(random.uniform(0, len(config.patterns)))]
        if random.random() < _paletteChangeProb :
            _paletteObj  = config.palettes[math.floor(random.random() * len (config.palettes))]
        ...
        # ###########################################################################
        # config.patternSequence.append([pattern, position, rotate, paletteToUse])
        # ###########################################################################
        ...
        _blockObject = Holder()
        _blockObject.pattern = pattern
        _blockObject.position = position
        _blockObject.rotate = rotate
        _blockObject.patternModel = pattern
        _blockObject.rotateAltBlock = rotate
        _blockObject.colOverlay = _paletteObj.colOverlay
        _blockObject.linecolOverlay = _paletteObj.linecolOverlay
        _blockObject.linecolOverlay2 = _paletteObj.linecolOverlay2

        config.blockSequence.append(_blockObject)

        i += 1
        iterateCount += 1

    print("----------------------------------------------")
    print(f"New sequence {len(config.patternSequence)} ==> {config.patternSequence}")
    # print(("Using start pattern {}").format(config.patternModel))
    print("----------------------------------------------")


# ------------------ buildPatterns -----------------------
def buildPatterns(arg=0):

    c = round(random.uniform(1, 4))

    if c == 1:
        if config.numRowsRandomize:
            print("rebuildPattern Called. Option 1")
            print("----------------------------------------------")
    ...
    if c == 2 or (random.random() < config.changePaletteWhenRebuildProb):
        print("rebuildPattern Called. Option 2")
        print("----------------------------------------------")
    ...
    if c >= 3:
        print("rebuildPattern Called. Option 3")
        print("----------------------------------------------")
        mekePatternSequence(config)
...
def iterate():
    ...
    config.bgColor = tuple(round(a * config.brightness) for a in (config.bgColor))
...
def main(run=True):
    ...
    firstPalette = config.palettes[0]
    config.bgColor = tuple(round(a * config.brightness) for a in (firstPalette.colOverlay.currentColor))
    ...
    # ###########################################################################
    print("Main() does initial call to rebuild pattern sequence")
    mekePatternSequence(config)
    # ###########################################################################
