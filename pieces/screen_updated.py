                ...
                for x in range(startX, endX, 1):
                    ...
                    if i > 0 and random.random() < config.probDrawLines * config.probabilityMultiplier:
                        ...
                        if samplePoint[0] < config.canvasWidth and samplePoint[1] < config.canvasHeight and samplePoint[0] > 0 and samplePoint[1] > 0:
                            ...
                            colorSampleColor = tuple(int(round(c * random.uniform(0.1, 1.8))) for c in colorSample)

                            # Once in a little while, the color is just random
                            if random.random() < config.randomColorSampleProb:
                                colorSampleColor = colorutils.getRandomRGB(random.random() * config.brightness)
...
def iterate():
    ...
    if random.random() < config.unpauseProb / 2 and config.probabilityMultiplierRange > 1 and not config.pausing:
    ...
    if random.random() < config.crackChangeProb:
        ...
        # pieceLogger (c,len(config.crackArray))
    ...
    if random.random() < config.pixelSortProbOn:
        ...
        config.randomColorProbabilty = random.uniform(0.01, 0.2)
