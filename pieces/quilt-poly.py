...
def main(run=True):
    ...
    config.transitionStepsMin = float(
        workConfig.get("quilt-polys", "transitionStepsMin")
    )
    config.transitionStepsMax = float(
        workConfig.get("quilt-polys", "transitionStepsMax")
    )

    # Some triangles will re-draw like a tick - on triangles quilt
    config.resetTrianglesProb = float(
        workConfig.get("quilt-polys", "resetTrianglesProb")
    )

    # The probability that at the beginning of a new quilt image the size of the
    # elements will change
    config.resetSizeProbability = float(
        workConfig.get("quilt-polys", "resetSizeProbability")
    )
    ...
    config.brightnessFactorDark = float(
        workConfig.get("quilt-polys", "brightnessFactorDark")
    )
