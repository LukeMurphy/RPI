...
i75 = Interstate75(
    display=DISPLAY_INTERSTATE75_64X64, stb_invert=False, panel_type=Interstate75.PANEL_GENERIC)
display = i75.display
...
while True:
    ...
    display.text(DAYS[current_t[3]], (WIDTH // 2) - (day_length // 2) + SHADOW_OFFSET, 2 + SHADOW_OFFSET, WIDTH, TEXTSCALE)

    display.set_font("bitmap8")
    display.text(f"{current_t[1]}/{current_t[2]}", (WIDTH // 2) - (date_length // 2) + SHADOW_OFFSET + 4, 33 + SHADOW_OFFSET, WIDTH, TEXTSCALE)

    # Main Text
    display.set_font("bitmap8")
    display.set_pen(WHITE)
    #display.text(DAYS[current_t[3]], (WIDTH // 2) - (day_length // 2), 2, WIDTH, TEXTSCALE)

    display.set_pen(RED)
    display.set_font("bitmap8")
    #display.text(str(current_t[2]), (WIDTH // 2) - (date_length // 2) + 4, 55, WIDTH, TEXTSCALE)
