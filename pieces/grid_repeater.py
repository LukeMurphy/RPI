    ...
    if config.figureType == "boxes":
        ...
        if config.elementNumber > 1:
            draw.rectangle(
                (
                    config.blockWidth / 2,
                    config.blockHeight / 2,
                    config.blockWidth / 2 + config.boxSize,
                    config.blockHeight / 2 + config.boxSize,
                ),
                fill=(config.lineBColor),
            )
        if config.elementNumber > 2:
            draw.rectangle(
                (
                    3 * config.blockWidth / 4,
                    3 * config.blockHeight / 4,
                    3 * config.blockWidth / 4 + config.boxSize,
                    3 * config.blockHeight / 4 + config.boxSize,
                ),
                fill=(config.lineCColor),
            )
    else:
        if config.elementNumber > 2:
            draw.line(
                (0, 1, config.blockWidth, config.blockHeight + 1),
                fill=config.lineCColor,
            )
            draw.line(
                (0, 0, config.blockWidth, config.blockHeight), fill=config.lineCColor
            )
        if config.elementNumber > 1:
            draw.line(
                (config.blockWidth / 2, 0, config.blockWidth / 2, config.blockHeight),
                fill=config.lineBColor,
            )
            draw.line(
                (
                    config.blockWidth / 2 + 1,
                    0,
                    config.blockWidth / 2 + 1,
                    config.blockHeight,
                ),
        ...
        draw.line(
            (0, config.blockHeight / 2, config.blockWidth, config.blockHeight / 2),
            fill=config.lineAColor,
        )
        draw.line(
            (
                0,
                config.blockHeight / 2 + 1,
                config.blockWidth,
                config.blockHeight / 2 + 1,
            ),
            fill=config.lineAColor,
