...
class WaveDeformer:
    ...
    def transform(self, x, y):
        y = y + self.config.waveAmplitude * math.sin((x + self.config.waveDeformXPos) / self.config.wavePeriodMod) * noise.pnoise2(math.sin(x), y / self.config.pNoiseMod)
        return x, y
    ...
    def getmesh(self, img):
        ...
        target_grid = [
            ...
            for x, y in itertools.product(
                range(0, self.w, self.config.wavegridspace),
                range(0, self.h, self.config.wavegridspace),
            )
        ]
...
def rebuildSections(config):
    ...
    if not config.diagonalMovement:
        sectionDisturbanceDirection = 1 if random.SystemRandom().random() < 0.5 else 0

    baseSpeed = config.baseSectionSpeed

    for i in range(config.numberOfSections):
        section = config.movingSections[i]
        section.sectionRotation = random.SystemRandom().uniform(-config.sectionRotationRange, config.sectionRotationRange)
        section.sectionPlacement = [
            round(random.SystemRandom().uniform(config.sectionPlacementXRange[0], config.sectionPlacementXRange[1])),
            round(random.SystemRandom().uniform(config.sectionPlacementYRange[0], config.sectionPlacementYRange[1])),
        ]
        section.sectionPlacementInit = [section.sectionPlacement[0], section.sectionPlacement[1]]
        section.sectionSize = [
            round(random.SystemRandom().uniform(config.sectionWidthRange[0], config.sectionWidthRange[1])),
            round(random.SystemRandom().uniform(config.sectionHeightRange[0], config.sectionHeightRange[1])),
        ]
        section.sectionSpeed = [
            random.SystemRandom().uniform(-baseSpeed, baseSpeed) / config.sectionSpeedFactorHorizontal,
...
def disturber(config):
    ...
    if not config.doSectionDisturbance or config.rebuildingPattern:
        return
    for i in range(config.numberOfSections):
...
def distortionsConfigs(config, workConfig):

    config.distortionConfigRef = config
    config.distortionworkConfigRef = workConfig

    config.dblockWidth = int(workConfig.get("distortionsConfigs", "blockWidth"))
    config.dblockHeight = int(workConfig.get("distortionsConfigs", "blockHeight"))

    config.imgcanvasOffsetX = int(workConfig.get("distortionsConfigs", "canvasOffsetX"))
    config.imgcanvasOffsetY = int(workConfig.get("distortionsConfigs", "canvasOffsetY"))

    config.useWaveDistortion = workConfig.getboolean("distortionsConfigs", "useWaveDistortion")
    config.waveAmplitude = float(workConfig.get("distortionsConfigs", "waveAmplitude"))
    config.wavePeriodMod = float(workConfig.get("distortionsConfigs", "wavePeriodMod"))
    config.wavegridspace = int(workConfig.get("distortionsConfigs", "wavegridspace"))
    ...
    config.changeDisturbanceSetProb = float(workConfig.get("distortionsConfigs", "changeDisturbanceSetProb"))
    config.rebuildPatternProbability = float(workConfig.get("distortionsConfigs", "rebuildPatternProbability"))
    ...
    config.stableSectionsMin = int(workConfig.get("distortionsConfigs", "stableSectionsMin"))
    config.stableSectionsMax = int(workConfig.get("distortionsConfigs", "stableSectionsMax"))
    config.stableSectionsMinWidth = int(workConfig.get("distortionsConfigs", "stableSectionsMinWidth"))
    config.stableSectionsMinHeight = int(workConfig.get("distortionsConfigs", "stableSectionsMinHeight"))
    config.stableSectionsChangeProb = float(workConfig.get("distortionsConfigs", "stableSectionsChangeProb"))
