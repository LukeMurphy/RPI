...
def main(run=True):
    ...
    try:
        config.legacyImageSpriteModule = workConfig.getboolean("repeater", "legacyImageSpriteModule")
        from modules.imagesprite_v1 import ImageSprite
    except Exception as e:
        printError(e)
        config.legacyImageSpriteModule = False
        from modules.imagesprite import ImageSprite
    ...
    imageList.extend(iter(config.imageList.split(',')))
    for _ in range(config.unitCount):
...
def runWork():
    ...
    while config.isRunning == True:
        ...
        time.sleep(config.directorController.loopDelay)
...
def iterate(n=0):
    ...
    if random.random() > 0.9985:
        colorModeDirectional = colorModeDirectional != True
