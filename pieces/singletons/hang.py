...
def init():
	...
	config.body = [
		...
		(5, 2, 5, 3),
		(5, 3, 5, 5),
		(5, 3, 3, (random.uniform(0, 5))),
		(5, 3, 7, (random.uniform(0, 5))),
		(5, 5, 4, (random.uniform(7, 9))),
		(5, 5, 6, (random.uniform(7, 9))),
	...
	config.draw.rectangle(
		(
			0,
			0,
			config.screenWidth + abs(config.imageXOffset),
			config.screenHeight + abs(config.imageYOffset),
		),
		fill=(
			5,
			5,
			5 + int(round(random.random() * 10)),
	...
	if len(config.alphabetLeft) > 0 and config.wordNotFound == True:
		...
		if (pos) != -1:
			...
			config.font = ImageFont.truetype(
				config.path + "/assets/fonts/freefont/FreeSansBold.ttf", 20
			)
			# drawText(config.textPosX + pos * 10,config.textPosY, char)
			# drawText(config.textPosX + pos * 10,config.textPosY, "*")

			xPos = config.letterxOffset + random.uniform(
				config.hBuffer, config.screenWidth - config.hBuffer
			)
			yPos = config.letteryOffset + +random.uniform(
				config.vBuffer / 3, config.screenHeight - config.vBuffer
			)
			...
			xPos = config.guessxOffset + random.uniform(
				config.hBuffer, config.screenWidth - config.hBuffer
			)
			yPos = config.guessyOffset + random.uniform(
				config.vBuffer / 3, config.screenHeight - config.vBuffer
			)
			...
			if len(config.guessed) <= len(config.scaffolding):
				...
				config.draw.line(
					tuple(15 * x + config.xOffset for x in coords),
					fill=(gray1, gray1, gray1),
				)

				if len(config.guessed) >= len(config.scaffolding):
					config.draw.line(
						tuple(15 * x + config.xOffset for x in coords),
						fill=(gray2, gray2, gray2),
					)
			...
			# Draw the body
				...
				if index == 0:
					...
					config.draw.line(
						tuple(15 * x + config.xOffset for x in coords), fill=(red, 0, 0)
					)
