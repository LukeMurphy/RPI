	...
	def drawOval(self):
		...
		for i in range(0, self.numLines):
			self.r = round(
				0.2
				* math.pi
				* math.cos(math.sin(i * self.angleRads + self.angle))
				* 255
			)
			self.g = round(math.cos(math.pi + i * self.angleRads + self.angle) * 255)
			self.b = round(
				math.sin(math.pi + i * self.angleRads + 1 * self.angle) * 255
			)
			...
			for n in range(0, self.linesPerBand):
				...
				box = [
					(xPos, yPos),
					(
						self.objWidth / 2 + i * self.expRate + self.x0 + n,
						self.objHeight / 2 + i * self.expRate + self.y0 + n,
					),
				]
