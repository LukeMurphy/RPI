...
def main(run=True):
	...
	config.crawlRepeatDistanceFactor = float(
		workConfig.get("txtdisplay", "crawlRepeatDistanceFactor")
	)

	config.txtBoxHtMultuiplier = float(
		workConfig.get("txtdisplay", "txtBoxHtMultuiplier")
	)
	...
	config.font1 = ImageFont.truetype(
		config.path + "/assets/fonts/freefont/FreeSerifBold.ttf", config.fontSize
	)
	config.font2 = ImageFont.truetype(
		config.path + "/assets/fonts/freefont/ComicSansMSBold.ttf", config.fontSize
	)
	config.font3 = ImageFont.truetype(
...
def randomizeVelocities():
	if random.random() > 0.5:
		config.vx = int(config.xVelocityRange * random.random())
	if random.random() > 0.5:
		config.vy = int(config.yVelocityRange * random.random())
...
def iterate(n=0):
	...
	if config.vx == 0 and config.vy == 0:
		randomizeVelocities()
	if config.vy == 0:
		randomizeVelocities()
	...
	config.textDraw.rectangle(
		(0, 128, config.txtBoxWd, config.txtBoxHt), fill=(0, 0, 10, 10)
	)
	...
	fontToUse = config.font3
	config.crawlDraw.rectangle(
		(0, 0, config.screenHeight, config.crawlHeight), fill=config.txtBackgroundColor
	)
	...
	config.crawlDraw.text(
		(config.crawlPosx, config.crawlPosy),
		config.crawlText,
		config.fontColor1,
		font=fontToUse,
	)
	...
	###### Paste in crawl
	config.image.paste(
		imgCrawl, (config.crawlPositionX, config.crawlPositionY), imgCrawl
	)
