...
def iterate(n=0):
    ...
    if config.allPause == True:
        ...
        _extracted_from_iterate_25(currentAnimation, config, bgColor)
    ...
    if random.random() < config.compositionModeChangeProb:
        _extracted_from_iterate_208(config)
    ...
    if config.animationController.advance == True:
        ...
        if config.playInOrder == True:
            ...
            if config.currentAnimationIndex >= len(config.animations):
                config.currentAnimationIndex = 0
            print(
                f"Next Animation : {str(config.animations[config.currentAnimationIndex].name)}"
            )
        else:
            choice = math.floor(random.uniform(0,len(config.animations)))
            config.currentAnimationIndex = choice
            print(f"Next Animation : {str(config.animations[choice].name)}")
...
def _extracted_from_iterate_208(config):
    config.compositionMode = math.floor(random.uniform(0,4))
    print(f"New composition mode: {str(config.compositionMode)}")
    if config.compositionMode == 0 :
        config.overlayDraw.rectangle((0,0,config.canvasWidth, config.canvasHeight), fill= (0,0,0,255))
    if config.compositionMode == 1 :
...
# TODO Rename this here and in `iterate`
def _extracted_from_iterate_25(currentAnimation, config, bgColor):
    ...
    if config.allPause == False:
        ...
        _extracted_from_iterate_66(anim, currentAnimation, config)
...
# TODO Rename this here and in `iterate`
def _extracted_from_iterate_66(anim, currentAnimation, config):
