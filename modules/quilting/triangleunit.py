...
class Unit:
	...
	def setupSquareWithTriangles(self):
		...
		self.sqrPoints.append(
			(self.xPos + xRan + self.blockLength / 2, self.yPos + yRan)
		)
		...
		self.sqrPoints.append(
			(self.xPos + xRan, self.yPos + self.blockHeight / 2 + yRan)
		)
		self.sqrPoints.append(
			(
				self.xPos + xRan + self.blockLength / 2,
				self.yPos + self.blockHeight / 2 + yRan,
			)
		)
		self.sqrPoints.append(
			(self.xPos + xRan + self.blockLength, self.yPos + self.blockHeight + yRan)
		)
		...
		self.sqrPoints.append(
			(
				self.xPos + xRan + self.blockLength / 2,
				self.yPos + self.blockHeight + yRan,
			)
		)
		self.sqrPoints.append(
			(self.xPos + xRan + self.blockLength, self.yPos + self.blockHeight + yRan)
	...
	def update(self):
		for i in range(0, self.triangleUnits):
			if random.random() > self.config.colorPopProb:
				...
				self.triangles[i][2] = tuple(
					round(a * self.brightness)
					for a in self.triangles[i][1].currentColor
				)
			else:
