...
class ParticleSystem:
	...
	def setUp(self):
		...
		self.maxRadius = math.sqrt(config.canvasWidth*config.canvasWidth +
								   config.canvasHeight*config.canvasHeight) * 2.5
		...
		for q in range(0, self.p):
			...
			if (
				...
				try:
					...
					if ref.sizeNum == 2:
						config.draw.rectangle((round(xDisplayPos), round(yDisplayPos), round(
							xDisplayPos)+1, round(yDisplayPos)+0), fill=(r, g, b, 255))
					else:
						config.draw.rectangle((round(xDisplayPos), round(yDisplayPos), round(
							xDisplayPos)+0, round(yDisplayPos)+0), fill=(r, g, b, 255))
...
def drawBands(p):
	...
	for i in range(0, p.bands):
		...
		if i == 0 or i == 12:
			config.draw.ellipse((x0, y0, x1, y1), fill=(rBase2, gBase2, bBase, round(a)))
		rBase += rDiff
		gBase += gDiff
		bBase += bDiff
