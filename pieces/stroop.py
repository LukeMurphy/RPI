...
def stroopSequence():
    ...
    # Default
    if choice == 1:
        colorWord, colorOfWord, directionStr = "YELLOW", (255, 0, 225), directionStr
    if choice == 2:
        colorWord, colorOfWord, directionStr = "VIOLET", (230, 225, 0), directionStr
    if choice == 3:
        colorWord, colorOfWord, directionStr = "RED", (0, 255, 0), directionStr
    if choice == 4:
        colorWord, colorOfWord, directionStr = "BLUE", (225, 100, 0), directionStr
    if choice == 5:
        colorWord, colorOfWord, directionStr = "GREEN", (255, 0, 0), directionStr
    if choice >= 6:
        colorWord, colorOfWord, directionStr = "ORANGE", (0, 0, 200), directionStr
...
def stroop():
    ...
    font = ImageFont.truetype(
        config.path + "/assets/fonts/freefont/FreeSansBold.ttf", stroopFontSize
    )
    ...
    bbox = font.getbbox(colorWord)
    pixLen = [bbox[2], bbox[3]+bbox[1]]

    dims = [pixLen[0], pixLen[1]]
    if dims[1] < config.tileSize[0]:
        dims[1] = config.tileSize[0] + 2
    ...
    if direction == "Top" or direction == "Bottom":
        ...
        for letter in chars:
            ...
            for i in range(1, shadowSize):
                draw.text(
                    (indent + -i, -i + counter * (stroopFontSize - 5)),
                    letter,
                    (0, 0, 0),
                    font=font2,
                )
    ...
    if direction == "Left" or direction == "Right":
        # Left Scroll
        vPadding = int(0.75 * config.tileSize[0])
        config.image = PIL.Image.new("RGBA", (dims[0] + vPadding, dims[1]))
        draw = ImageDraw.Draw(config.image)
        iid = config.image.im.id
        draw.rectangle(
            (0, 0, config.image.size[0] + config.tileSize[0], config.screenHeight),
            fill=bgColor,
        )
        ...
        if higherVariability:
            vOffset += int(
                random.uniform(-config.tileSize[0] / 8, config.tileSize[0] / 8)
            )
        ...
        if direction == "Left":
            ...
            end = config.screenWidth - int(
                random.uniform(-wd / 2, wd)
            )  # + config.image.size[0]
            dx = -steps
            x = start
