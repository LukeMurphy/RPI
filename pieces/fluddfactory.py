...
class Fludd:
    ...
    def transition(self):
        ...
        self.xPos2 = max(self.xPos1 + self.symBoxWidth-4, self.xPos1)
        self.yPos2 = max(self.yPos1 + self.symBoxHeight-4, self.yPos1)
    ...
    def reDraw(self):
        ...
        if self.varianceMode == "independent":
            ...
            self.symBoxWidth = self.boxMax - self.svarw * 2
            self.symBoxHeight = self.boxHeight - self.svarh  * 2
            self.xPos1 = self.boxMax - self.symBoxWidth
            self.yPos1 = self.boxHeight - self.symBoxHeight
            self.xPos2 = self.xPos1 + self.symBoxWidth
            self.yPos2 = self.yPos1 + self.symBoxHeight

            self.xPos2 = max(self.xPos1, self.xPos2)
            self.yPos2 = max(self.yPos1, self.yPos2)
            self.draw.rectangle(
                (self.xPos1, self.yPos1, self.xPos2, self.yPos2),
...
def redraw():
    ...
    for r in range(config.rowsOfSquares):
        for c in range(config.colsOfSquares):
            fluddSquare = fludds[squareCount]
            fluddSquare.transition()
            config.image.paste(
                fluddSquare.tempImage,
                (c * (config.boxWidth + 4), r * (config.boxHeight +  4)),
                fluddSquare.tempImage,
            )
            squareCount += 1
