...
class Line:
    ...
    def __init__(self, config, setLineAttributes=True):
        ...
        self.direction = -2 if random.SystemRandom().random() > 0.5 else 2
        # self.direction = 2
        self.pointArray = [[0, 0]]
    ...
    def setLineAttributes(self):
        # print("new")
        self.angle = random.SystemRandom().uniform(-math.pi / 2, math.pi / 2)
        ...
        x = random.SystemRandom().uniform(0, config.canvasWidth)
        y = random.SystemRandom().uniform(0, config.canvasHeight)

        r = round(random.SystemRandom().uniform(0, 3))
        while side == r:
            r = round(random.SystemRandom().uniform(0, 3))
    ...
    def drawLine(self):
        ...
        s = lng * self.widthMultiplier / (self.branchCount / 2 + 1)
        ...
        w = (config.initWidth - self.branchCount) * 0.75
        ...
        tempImageDraw.polygon(
            ((x0, y0), (x1, y1), (x1b, y1b), (x0b, y0b)),
            outline=None,
            fill=self.lineColor,
        )
...
def showLines():
    ...
    for i in range(numberOfLines):
        ...
        # Branch a line
        if ref.done and ref.branchCount < config.branchCountLimit:
            lng = len(ref.pointArray)
            if lng >= 1:
                point = round(lng / 2)
                midPoint = ref.pointArray[point]
                lastPoint = ref.pointArray[lng - 1]

                ref.pointArray = []

                # if random.SystemRandom().random() < config.branchAtTipProb :
                # 	midPoint = lastPoint
    ...
    for ln in newLines:
        ...
        newLine.angle = ln[0]
        newLine.lastPoint = ln[1]
        newLine.nextPoint = ln[1]
        newLine.done = False
        newLine.lineNumber = ln[4]
        newLine.lineColor = ln[2]
        newLine.branchCount = ln[3]
        newLine.direction = ln[5]
    ...
    for n, ln in enumerate(config.linesArray):
        if ln.done:
...
def setUp():
    ...
    for i in range(config.simultaneousLines):
        lng = Line(config, False)
        lng.lineNumber = i
        lng.lineColor = colorutils.randomColorAlpha(1.0, 50, 255)
        lng.segmentLength = config.segmentLength
        lng.widthMultiplier = config.widthMultiplier
        lng.setLineAttributes()
        # print(l.lastPoint)
        config.linesArray.append(lng)
        config.lineCount += 1
