    ...
    for root, dirs, files in os.walk(configPath, topdown=False):
        for name in files:
            ...
            if name.endswith(".cfg") and not name.endswith(".py") and name != ".DS_Store":
                ...
                if "asset_configs" not in fullPath and "/marks" not in fullPath and "/textures" not in fullPath and "/colors" not in fullPath and "/disturbances" not in fullPath and "/palettes" not in fullPath:
