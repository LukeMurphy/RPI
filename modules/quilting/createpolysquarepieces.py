...
def createPieces(config, refresh=False):
	...
	for rows in range(0, config.blockRows):
		mult = 0
		if config.elongation == 1:
			mult = 0
		if config.elongation == 2:
			mult = 3
		if config.elongation == 3:
			mult = 4
		if config.elongation == 4:
			mult = 6
		if config.elongation == 5:
			mult = 9

		rowStart = (
			rows * config.blockHeight * 3
			+ rows * config.gapSize
			- rows * mult * config.blockHeight / config.elongation
			- config.blockHeight
		)
