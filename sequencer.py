...
def timeChecker(sequenceConfig):
    ...
    if sequenceConfig.currentTime - sequenceConfig.startTime > sequenceConfig.currentPieceDuration:
        ...
        print(f"Current PID = {currentPID}")
        # os.system("ps -ef | pgrep -f player | xargs kill -9;")

        runString = f"{sequenceConfig.commadStringPyth} {sequenceConfig.workListDirectory}{sequenceConfig.workList[pieceToPlay][0]}&"

        os.system(
            f"{sequenceConfig.commadStringPyth} {sequenceConfig.workListDirectory}{sequenceConfig.workList[pieceToPlay][0]}&"
        )
        ...
        print(f"==========> count play : {str(sequenceConfig.playCount)} / {str(sequenceConfig.repeatCountTrigger)}")
        if sequenceConfig.playCount >= 1 or str(currentPID) != "":
            print("==========> Ending back window in 2 seconds")
            time.sleep(2)
            os.system(f"kill -9 {str(currentPID)}")
...
def loadWorkConfig(work, sequenceConfig):
    ...
    argument = f"{config.path}/configs/{sequenceConfig.workListDirectory}{work[0]}"

    print(f"{bcolors.WARNING}** ")
    print(f"Sequencer: {work[0]}:{argument}")
    print(bcolors.ENDC)
...
def loadSequenceFile():
    ...
    if args.cfg != None:
        ...
        print(f"{bcolors.OKBLUE}** {str(args)}{bcolors.ENDC}")
