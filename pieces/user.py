...
def drawUser(n = 0) :
	...
	onColor = (0,0,100,255)
	oColor  = (int(400 * config.brightness),int(random.uniform(50,300) * config.brightness),int(0 * config.brightness), 255)
	...
	if (random.random() > .8) :
		...
		if(n!=1 or fixed == True) :
			draw.ellipse((mx1,my1- mh/2,mx2,my2 + mh/2), fill=(int(180 * config.brightness),int(80 * config.brightness),int(80 * config.brightness), a), outline=1)
...
def callBack() :
	global config, fixed
	fixed = True if (random.random() > .5 ) else False
	userAnimator()
