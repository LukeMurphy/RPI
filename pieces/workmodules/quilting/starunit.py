...
class Unit:
	...
	def __init__(self, config):
		...
		self.triangles = [
			[[], coloroverlay.ColorOverlay(False), []] for i in range(0, 8)
		]
	...
	def setupFullSquares(self):
		...
		self.sqrPoints.append(
			(self.xPos + self.blockLength / 2, self.yPos + self.blockHeight / 2)
		)
		self.sqrPoints.append(
		...
		self.sqrPoints.append(
			(self.xPos + self.blockLength / 2, self.yPos + self.blockHeight)
		)
		self.sqrPoints.append(
			(self.xPos + self.blockLength, self.yPos + self.blockHeight)
		)
	...
	def setupTriangles(self):
		...
		self.starPoints.append(
			(
				self.xPos
				+ random.uniform(self.blockLength / 4, 3 * self.blockLength / 4),
				self.yPos,
			)
		)
	...
	def update(self):
		...
		if random.random() > self.config.colorPopProb:
			...
			self.fillColor = tuple(
				round(a * self.brightness) for a in self.colOverlay.currentColor
			)
		else:
		...
		if random.random() > self.config.resetTrianglesProb:
			self.setupTriangles()
	...
	def drawUnitTriangles(self, orientation=0):
		...
		for side in range(0, 4):
			...
			if self.squareNumber == 0:
				if c == 0 or c == 1:
					fillColor = tuple(
						[round(i / self.darkeningFactor) for i in fillColor]
					)
