...
def ringLinesNoLoop():
    ...
    if tDelta >= config.timeToChangeRings:
        ...
        config.draw.ellipse(
            (
                config.xOffset - radius,
                config.yOffset - radius,
                config.xOffset + radius,
                config.yOffset + radius,
            ),
            fill=getColor(250, 250, 250, 255),
            outline=None,
        )
...
def renderRingLines():
    ...
    for col in range(config.numRings):
        ...
        yChange1 = (
            noise.pnoise2(x + config.scrollx, y + config.scrolly) * config.amplitude
            + row
        )
        yChange2 = (
            noise.pnoise2(x + config.scrollx, y + config.scrolly)
            * config.amplitude
            / config.gDelta
            + row
        )
        ...
        xPos = (
            radialFactor * (config.radiusMin + yChange1) * math.cos(col * rads) * 2
            + config.xOffset
        )
        yPos = (
            radialFactor * (config.radiusMin + yChange1) * math.sin(col * rads) * 2
            + config.yOffset
        )
        ...
        config.draw.rectangle(
            (xPos - config.markSize, yPos - config.markSize, xPos + 0, yPos + 0),
            fill=clrToUse2,
            outline=None,
        )
        ...
        xPos = (
            radialFactor * (config.radiusMin + yChange2) * math.cos(col * rads) * 2
            + config.xOffset
        )
        yPos = (
            radialFactor * (config.radiusMin + yChange2) * math.sin(col * rads) * 2
            + config.yOffset
        )
        config.draw.rectangle(
            (xPos - config.markSize, yPos - config.markSize, xPos + 0, yPos + 0),
        ...
        xPos = (
            radialFactor * (config.radiusMin + yChange3) * math.cos(col * rads) * 2
            + config.xOffset
        )
        yPos = (
            radialFactor * (config.radiusMin + yChange3) * math.sin(col * rads) * 2
            + config.yOffset
        )
        config.draw.rectangle(
            (xPos - config.markSize, yPos - config.markSize, xPos + 0, yPos + 0),
...
def rings():
    ...
    for row in range(config.numRings):
        ...
        for col in range(points):
            ...
            yChange1 = (
                noise.pnoise2(
                    x / config.rowFactor, (y + config.scroll) / config.colFactor / 1, 1
                )
                * config.amplitude
                + row
            )
            yChange2 = (
                noise.pnoise2(
                    x / config.rowFactor,
                    (y + config.scroll) / config.colFactor / gDelta,
                    1,
                )
                * config.amplitude
                + row
            ...
            yChange3 = (
                noise.pnoise2(
                    x / config.rowFactor,
                    (y + config.scroll) / config.colFactor / bDelta,
                    1,
                )
                * config.amplitude
                + row
            ...
            if (
                x > 0
                and x < config.canvasWidth - config.xOffset
                and (y + yChange1) > 0
                and (y + yChange1) < config.canvasHeight
            ):
                doDraw = True
            ...
            if doDraw:
                if config.markSize == 1:
                    config.draw.rectangle(
                        (x, y + yChange1, x + 0, y + yChange1 + 0),
                        fill=getColor(255, 0, 100, 255),
                        outline=None,
                    )
                    config.draw.rectangle(
                        (x, y + yChange2, x + 0, y + yChange2 + 0),
                        fill=getColor(0, 255, 0, 255),
                        outline=None,
                    )
                    config.draw.rectangle(
                        (x, y + yChange3, x + 0, y + yChange3 + 0),
                        fill=getColor(0, 0, 255, 255),
                        outline=None,
                    )
                else:
                    config.draw.ellipse(
                        (
                            ...
                            y + yChange2,
                            x + config.markSize,
                            y + yChange1 + config.markSize,
                        ),
                        fill=getColor(255, 0, 0, 255),
                        outline=None,
                    )
                    config.draw.ellipse(
                        (
...
def ringScribbles():
    ...
    for col in range(totalPoints):
        ...
        xPos = (
            Rx * math.cos(rotationTheta) * math.cos(a)
            - Ry * math.sin(rotationTheta) * math.sin(a)
            + config.xOffset
        )
        yPos = (
            Ry * math.cos(rotationTheta) * math.sin(a)
            - Ry * math.sin(rotationTheta) * math.cos(a)
            + config.yOffset
        )
        ...
        clrToUse2 = (255, 0, 0, config.redAlpha)
        clrToUse2 = colorutils.getRandomColorHSV(
            config.line_1_minHue,
            config.line_1_maxHue,
            config.line_1_minSaturation,
            config.line_1_maxSaturation,
            config.line_1_minValue,
            config.line_1_maxValue,
            0,
            0,
            config.redAlpha,
            brtns,
...
def wavey():
    ...
    for col in range(0, config.canvasWidth * 1, 2):
        ...
        for row in range(0, config.canvasHeight, 8):
            ...
            x = round(
                noise.pnoise2(
                    (col + config.scroll) / config.colFactor / 3,
                    (y) / config.rowFactor / 1,
                    1,
                )
                * config.amplitude
                + col
            ...
            r = round(math.sin((y / config.colFactor) + 0.1) * 200)
            g = round(math.sin((col / config.rowFactor) + 0.1) * 100)
            # g = round(math.sin((x/config.rowFactor)+.1) * 200)
            b = round(math.sin((x / config.rowFactor) + 0.1) * 100)
            # config.draw.rectangle((x, y, x+1, y+1), fill=(r,g,b,150))
            if col != 0:
                config.draw.line(
                    (lastx[0], lasty[0], x, y), fill=getColor(r, g, b, 150)
                )
            lastx = [x, x, x]
            lasty = [y, y, y]
        # octv += 1
...
class Dyad:
    ...
    def update(self):
        ...
        distancex = abs((self.P1.x - self.P2.x))
        ...
        if not self.P1.fixed:
            if not self.P1.endPoint :
                self.P1.velX += -self.k * (self.P1.x - self.P2.x)
                self.P1.velX *= self.damp * self.dampingFactor
            if distancex < maxDistance:
                self.P1.x += self.P1.velX * self.speedModifier
                self.P1.y += self.P1.velY * self.speedModifier
        else:
            self.P1.velX = 0
            # self.P1.fixed = True

        if not self.P2.fixed :
            if not self.P2.endPoint :
                self.P2.velX += -self.k * (self.P2.x - self.P1.x)
                self.P2.velX *= self.damp * self.dampingFactor

            if distancex < maxDistance:
                self.P2.x += self.P2.velX * self.speedModifier
                self.P2.y += self.P2.velY * self.speedModifier
        else:
            self.P2.velX = 0
            # self.P2.fixed = True
...
def makeBarColors():
    ...
    for col in range(totalColumns):
        ...
        barColor = config.palette.chooseFromPalette(
            config.springColorAlpha, config.brightness
        )
        ...
        if col > 0:
            ...
            if col == totalColumns - 1:
                ...
                dyad.P2.x = (
                    col * config.colInterval
                    + config.springXPosStart
                    - config.springXPosEnd
                )
...
def springs():
    ...
    for u in range(numUnits):
        ...
        unitRef.update()
        # dW = 2
        xPos1 = unitRef.P1.x - 4
        yPos1 = unitRef.P1.y + fixedYPos
        xPos2 = unitRef.P2.x + 4
        yPos2 = unitRef.P2.y + fixedYPos + fixedYHeight  # * unitRef.k
...
def waves():
    ...
    for row in range(config.waveLines):
        ...
        for col in range(
            0, config.canvasWidth + config.colInterval, config.colInterval
        ):
            x = col
            xPos2 = col
            # moving
            ...
            # y = f + config.yOffset + config.lineFactor * 1
            ...
            if col != 0 and row == 0:
                ...
                xPos2 = round(x + d / 2)
                config.draw.rectangle(
                    (xPos1, yPos1, xPos2, yPos2), fill=config.barColors[colorCount]
                )
                colorCount += 1
...
def iterate():
    ...
    if config.useDrawingPoints:
        ...
        if config.useWaveDistortion:
            ...
            tempImage = config.image.filter(ImageFilter.GaussianBlur(radius=0.75))
...
def main(run=True):
    ...
    try:
        ...
        config.changeColorSpringsProb = float(
            workConfig.get("noisescroller", "changeColorSpringsProb")
        )
        config.changeSpringsProb = float(
            workConfig.get("noisescroller", "changeSpringsProb")
        )
        config.springColorAlpha = int(
            workConfig.get("noisescroller", "springColorAlpha")
        )
        ...
        config.changeColorSpringsProb = 0.001
        config.changeSpringsProb = 0.005
    ...
    if (
        config.function == "ringLines"
        or config.function == "ringLinesNoLoop"
        or config.function == "ringScribbles"
    ):
        ...
        config.animationDuration = int(
            workConfig.get("noisescroller", "animationDuration")
        )
        ...
        config.timeToChangeRings = int(
            workConfig.get("noisescroller", "timeToChangeRings")
        )
    ...
    for _ in range(config.waveLines):
    ...
    try:
        ...
        config.waveDeformXPosRate = float(
            workConfig.get("noisescroller", "waveDeformXPosRate")
        )
