...
def main(run=True):
    ...
    try:
        config.doingRefreshCount = int(workConfig.get("images", "doingRefreshCount"))
    except Exception as e:
        print(str(e))
...
def performChanges():
    ...
    if random.random() < config.imageGlitchProb:
        config.imgLoader.image = config.imgLoader.image.rotate(random.uniform(-0.25, 0.25))
        config.glitchCount += 1

    config.workImage.paste(config.imgLoader.image.convert("RGBA"), (0, 0), config.imgLoader.image.convert("RGBA"))

    if random.random() < 0.1 and config.glitchCount < 3:
        config.imgLoaderAlt.image = config.imgLoaderAlt.image.rotate(random.uniform(0, 360))
        config.imgLoader.image.paste(
            config.imgLoaderAlt.image.convert("RGBA"),
            (round(random.uniform(0, config.canvasWidth)), round(random.uniform(0, config.canvasHeight))),
            config.imgLoaderAlt.image.convert("RGBA"),
        )
