...
def init():
    ...
    config.carcas_pixSortYOffset = int(
        workConfig.get("animals", "carcas_pixSortYOffset")
    )
    ...
    config.useColorOverlayTransitions = workConfig.getboolean(
        "animals", "useColorOverlayTransitions"
    )
    config.applyColorOverlayToFullImage = workConfig.getboolean(
        "animals", "applyColorOverlayToFullImage"
    )
    config.useScrollingBackGround = workConfig.getboolean(
        "animals", "useScrollingBackGround"
    )
    ...
    config.colorTransitionRangeMin = float(
        workConfig.get("animals", "colorTransitionRangeMin")
    )
    config.colorTransitionRangeMax = float(
        workConfig.get("animals", "colorTransitionRangeMax")
    )
    ...
    config.clrBlock = Image.new(
        config.workImage.mode, (config.canvasWidth, config.canvasHeight)
    )
    ...
    config.colOverlayA.randomRange = (
        config.colorTransitionRangeMin,
        config.colorTransitionRangeMax,
    )
    config.colOverlayA.colorA = tuple(
        int(a * config.brightness) for a in (colorutils.getRandomColor())
    )
...
def iterate():
    ...
    if (
        config.useColorOverlayTransitions == True
        and config.applyColorOverlayToFullImage == True
    ):
        # Color overlay on b/w PNG sprite
        # clrBlockDraw.rectangle((0,0, config.canvasWidth, config.canvasHeight), fill=(255,255,255))
        config.clrBlockDraw.rectangle(
