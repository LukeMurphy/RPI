...
class unit:
    def __init__(self, config):
        ...
        self.fillColor = colorutils.randomColorAlpha(config.brightness, 100,10)
        self.outlineColor = colorutils.randomColorAlpha(config.brightness, 100,10)
        self.objWidth = 50
        self.objWidthMax = 50
        self.objWidthMin = 50
...
def displayTest():
    ...
    config.imageTest1 = Image.new("RGBA", (200, 200))
    config.drawTest1 = ImageDraw.Draw(config.imageTest1)
    
    config.imageTest2 = Image.new("RGBA", (200, 200))
    config.drawTest2 = ImageDraw.Draw(config.imageTest2)

    config.imageTest3 = Image.new("RGBA", (200, 200))
    config.drawTest3 = ImageDraw.Draw(config.imageTest3)

    config.drawTest1.rectangle((0, 0, 100, 100), fill=(200,0,255,150), outline=colorBlock)
    config.drawTest2.rectangle((30, 30, 100, 100), fill=(200,250,0,150), outline=colorBlock)
    config.drawTest3.rectangle((0, 0, 100, 100), fill=(200,0,0,150), outline=colorBlock)
    
    imgCombo = ImageChops.add(config.imageTest2, config.imageTest3, scale=1.0, offset=0)
    
    config.image.paste(config.imageTest1,(120,120), config.imageTest1)
    config.image.paste(config.imageTest2,(150,150), config.imageTest2)
    config.image.paste(imgCombo,(200,200), imgCombo)
...
def main(run=True):
    ...
    try:
        config.rowsVals = (workConfig.get("base", "rowsToShow")).split(",")
    ...
    try:
        config.fontSize = int(workConfig.get("base", "fontSize"))
    except Exception as e:
        print(str(e))
        config.fontSize = 14
    try:
    ...
    try:
        config.showGrid = workConfig.getboolean("base", "showGrid")
    except Exception as e:
        print(str(e))
        config.showGrid = False
    try:
        fontColor2 = workConfig.get("base", "fontColor2").split(",")
    except Exception as e:
        print(str(e))
        fontColor2 = [0, 200, 0]
    ...
    try:
        config.useImage = (workConfig.getboolean("base", "useImage"))
        config.showAsOverLay = (workConfig.getboolean("base", "showAsOverLay"))
        config.imgPath = (workConfig.get("base", "imgPath"))
    except Exception as e:
        print(str(e))
    ...
    try:
        config.colorPalette = workConfig.getboolean("base", "colorPalette")
    except Exception as e:
        print(str(e))
