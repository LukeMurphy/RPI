...
def drawLines(arg="F"):
	...
	if arg == "F":
		...
		v = [
			d
			* math.log1p(math.fabs(currentAngle))
		...
		v = [
			16 * d * math.sin(a) * math.sin(a) * math.sin(a),
			d
			* (
				13 * math.cos(a)
				- 5 * math.cos(2 * a)
		...
		config.draw.line(
			(
				lastCoord[0] + origin[0],
				lastCoord[1] + origin[1],
				nextCoord[0] + origin[0],
				nextCoord[1] + origin[1],
			),
			fill=(int(config.brightness * 255), 0, 0),
		)
