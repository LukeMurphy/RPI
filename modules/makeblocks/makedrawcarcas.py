...
def drawCarcas():
	...
	polyToUse = [
		n + random.uniform(-config.pigglyWiggleToUse, config.pigglyWiggleToUse)
		for n in poly
	]
	...
	## resize to fit
	config.imageLayerTemp = config.imageLayerTemp.resize(
		(config.canvasWidth, config.canvasHeight)
	)

	## paste to the image layer
	config.imageLayerDraw.rectangle(
		(0, 0, config.canvasWidth, config.canvasHeight), fill=(0, 0, 0, config.alpha)
	)
	config.imageLayer.paste(
		config.imageLayerTemp, (config.xOffset, config.yOffset), config.imageLayerTemp
	)


def changePigglyWiggle():
	global config
	config.pigglyWiggleToUse = config.pigglyWiggle + int(
		random.uniform(0, config.pigglyWiggleVariance)
	)
	pass
