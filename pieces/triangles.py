...
class Bar:
    ...
    def remake(self):
        self.xSpeed = random.uniform(
            config.xSpeedRangeMin * config.direction,
            config.xSpeedRangeMax * config.direction,
        )
        ...
        self.barThickness = round(
            random.uniform(config.barThicknessMin, config.barThicknessMax)
        )
...
def iterate():
    ...
    for r in vr:
        ...
        # print(*zip(*polygon))
        if len(pnts) >= 2:
            config.draw.polygon(tuple(pnts), fill=(255, 0, 0), outline=(255, 255, 255))
...
def main(run=True):
    ...
    try:
        config.colorChangeProb = float(workConfig.get("params", "colorChangeProb"))
    except Exception as e:
        print(e)
        config.colorChangeProb = 0.003

    try:
        config.changeShapeProb = float(workConfig.get("params", "changeShapeProb"))
    except Exception as e:
        print(e)
        config.changeShapeProb = 0.001
    ...
    config.colorSetList = list((workConfig.get("params", "colorSets").split(",")))

    config.numberOfColorSets = len(config.colorSetList)
    for setName in config.colorSetList:
        cset = [float(i) for i in (workConfig.get("params", setName).split(","))]
    ...
    for _ in range(config.numberOfBars):
        bar = Bar()
