...
vec3 otherInterference2(void) {
	vec2 d = getCenterDistance();
	float r = d.x;
	vec3 res = vec3(
			sin(sin(r)*r*(time+tTimeMult)*rTimeMult)* brightness * 1.0  + min,
			sin(cos(r)*r*(time+tTimeMult)*gTimeMult)* brightness * 1.20  + min,
			sin(sin(r)*r*(time+tTimeMult)*bTimeMult)* brightness * 2.0 + min);
	return res;
}
