...
/* THIS IS TO TAKE THE ANIMATION OFF THE EDGE OF THE WINDOW
/* IT seems with whatever version there is a 5px rounded lighter
/* edge drawn around the sketch that plays havoc with the matrix tiling
/* so this basically gives a 5px buffer around the animation  --
/* note - the positioning of the screen crop by the video cards
/* should take into account this border
/* */
...
void settings() {
    ...
    windowWidth = parseInt(config.get("windowWidth")) + windowSideOffset * 2;
    windowHeight = parseInt(config.get("windowHeight"))+ windowSideOffset * 2;   
...
void draw() {
    ...
    if (cutups > 0) {
        ...
        image(ditherer.dither(crop1), windowSideOffset, windowSideOffset);
        g.removeCache(crop1);

        image(ditherer.dither(crop2), animationCanvaswidth*2 + windowSideOffset, windowSideOffset);
        g.removeCache(crop2); 

        pushMatrix();
        translate(animationCanvaswidth * 5, canvasheight);
        rotate(-HALF_PI*2);
        image(ditherer.dither(crop3), animationCanvaswidth*1 - windowSideOffset, -windowSideOffset);
        g.removeCache(crop3); 
        popMatrix();
        ...
        translate(animationCanvaswidth * 5, canvasheight);
        rotate(-HALF_PI*2);
        image(ditherer.dither(crop4), animationCanvaswidth*3- windowSideOffset, -windowSideOffset);
        g.removeCache(crop4);
        popMatrix();
    }
