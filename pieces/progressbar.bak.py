...
class ProgressBar :
	...
	def changeAction(self):
		...
		if(random.random() < self.pauseProbability) :
			...
			print("pauseProb",timeLeft, timeLeft-1)
			if (timeLeft >=1) :
				#self.hasPaused = False
				self.pausePoint = self.pausePoint + int(random.uniform(1,timeLeft-1))
				self.pausePoint = 30
				#print("pauseProb",self.pausePoint,timeLeft, timeLeft-1)
		'''
		...
		if(random.random() < self.completeProbability):
	...
	def done(self):
		...
		self.target = round(random.uniform(95,99))

		self.goBack = True if (random.random() < self.goBackwardsProb) else False
		self.goPast = True if (random.random() < self.goPastProb) else False
...
def iterate() :
	...
	if(pBar.firstRun and pBar.complete == False) :
		...
		pBar.paused = True
		firstRunCount+=config.countSpeed
	...
	if(pBar.complete) : 
		#time.sleep(int(random.uniform(pBar.minSleep,pBar.maxSleep)))
		completeCount+=config.countSpeed
		pBar.paused = True
...
def main(run = True) :
	...
	config.countSpeed = float(workConfig.get("progressbar", 'countSpeed'))
	...
	pBar.rateMultiplier = config.rateMultiplier
	...
	pBar.goPastProb = float(workConfig.get("progressbar", 'goPastProb'))
