...
from modules import badpixels, colorutils
...
def iterate():
	...
	draw.rectangle(
		(0, 0, projectedWidth, config.screenHeight),
		fill=tuple([int(a) for a in config.currentColor]),
	)
	config.canvasImage = ImageChops.multiply(config.canvasImage, segmentColorizer)

	for i in range(0, 3):
		if (
			(config.currentColor[i] - abs(config.rateOfColorChange[i]))
			<= config.colorB[i]
			<= (config.currentColor[i] + abs(config.rateOfColorChange[i]))
		):
	...
	for n in range(0, config.displayRows):
		...
		segment = config.canvasImage.crop(
			(
				n * config.screenWidth,
				0,
				segmentWidth + n * config.screenWidth,
				segmentHeight,
			)
		)
